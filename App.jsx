...
export default function App() {
  ...
  function exportBackup() {
    ...
    a.href = url;
    a.download = `trainings-tracker-backup-${todayStr()}.json`;
    a.click();
  ...
  function handleBackupFileSelect(e) {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (!data || !Array.isArray(data.days) || !Array.isArray(data.daySessions)) {
          setImportMessage({ type: "error", text: "Das ist keine gültige Backup-Datei." });
          return;
        }
        setDays(data.days);
        ...
        if (typeof data.vibrationEnabled === "boolean") setVibrationEnabled(data.vibrationEnabled);
        setActiveDayId(data.days[0]?.id);
        setHistoryDayId(data.days[0]?.id);
        setImportMessage({ type: "success", text: `Backup vom ${data.exportedAt ? new Date(data.exportedAt).toLocaleDateString("de-DE") : "unbekanntem Datum"} wiederhergestellt.` });
        setShowBackupMenu(false);
      } catch (err) {
        setImportMessage({ type: "error", text: "Datei konnte nicht gelesen werden." });
      }
    };
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 flex flex-col font-sans">
      <div className="max-w-md w-full mx-auto flex flex-col min-h-screen relative">
        ...
        <header className="px-5 pt-6 pb-4 flex items-center gap-2.5 border-b border-neutral-900">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <button
              ...
              className="w-8 h-8 rounded-md bg-neutral-900 flex items-center justify-center shrink-0 text-neutral-400 active:bg-neutral-800"
            >
              <Settings size={15} />
            </button>
          </div>
        ...
        {showBackupMenu && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-6">
            <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={() => setShowBackupMenu(false)} />
            <div className="relative w-full max-w-sm bg-neutral-900 border border-neutral-800 rounded-2xl p-5">
              <h2 className="text-[16px] font-bold mb-1">Backup</h2>
              <p className="text-[13px] text-neutral-400 leading-relaxed mb-5">
                Sichert alle Tage, Übungen und Trainings als Datei — falls dein Handy mal verloren geht oder du die
                App wechselst.
              </p>

              <button
                onClick={exportBackup}
                className="w-full flex items-center justify-center gap-2 bg-orange-500 text-neutral-950 rounded-xl py-3 text-[13px] font-bold active:bg-orange-400 mb-2.5"
              >
                <Download size={15} />
                Backup exportieren
              </button>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-neutral-800 text-neutral-200 rounded-xl py-3 text-[13px] font-bold active:bg-neutral-700 mb-4"
              >
                <Upload size={15} />
                Backup importieren
              </button>
              ...
              <p className="text-[11px] text-neutral-600 leading-relaxed mb-4">
                Import ersetzt alle aktuell gespeicherten Tage und Trainings durch den Stand aus der Datei.
              </p>

              <button
                onClick={() => setShowBackupMenu(false)}
                className="w-full text-[13px] font-semibold text-neutral-500 py-1"
              >
                Schließen
              </button>
            </div>
        ...
        {importMessage && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center px-6">
            <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={() => setImportMessage(null)} />
            <div className="relative w-full max-w-sm bg-neutral-900 border border-neutral-800 rounded-2xl p-5">
              <h2 className={"text-[16px] font-bold mb-2 " + (importMessage.type === "error" ? "text-red-400" : "text-emerald-400")}>
                {importMessage.type === "error" ? "Fehler beim Import" : "Wiederhergestellt"}
              </h2>
              <p className="text-[13px] text-neutral-400 leading-relaxed mb-5">{importMessage.text}</p>
              <button
                onClick={() => setImportMessage(null)}
                className="w-full bg-neutral-800 text-neutral-200 rounded-xl py-2.5 text-[13px] font-semibold active:bg-neutral-700"
              >
                OK
              </button>
            </div>
