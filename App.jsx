...
export default function App() {
  ...
  function saveDaySession() {
    const entries = Object.entries(activeDraft)
  ...
  function deleteDay(id, e) {
    ...
    const confirmed = window.confirm(
      `"${day?.name}" wirklich löschen?\n\nAlle zugehörigen Übungen dieses Tages gehen dabei verloren. Bereits gespeicherte Trainings bleiben im Verlauf erhalten.`
    );
    if (!confirmed) return;
  ...
  const totalDraftSets = Object.values(activeDraft).reduce((sum, arr) => sum + arr.length, 0);
  ...
  const weekCount = daySessions.filter((s) => {
                ...
                <>
                  ...
                  {totalDraftSets > 0 && (
                    <div className="flex gap-2 mt-2">
                      ...
                      >
                        {savedFlash ? "Gespeichert ✓" : `${activeDay.name}-Tag speichern`}
                      </button>
                      <button
